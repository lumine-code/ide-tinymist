...
const TYPST_SCOPE = ["source.typst"];
// Feature switches are grammar-scoped. Read through the Typst scope so a base
// value cannot make the server disagree with the per-editor feature router.
const setting = (key) => lumine.config.get(`ide-tinymist.${key}`, { scope: TYPST_SCOPE });
...
const tinymistOptions = () => ({
  ...
  // Empty means "beside the document", and Tinymist spells that as an absent
  // value: an empty pattern is a pattern, which it resolves to a relative path
  // and then refuses, failing every export with "output path is relative".
  ...
  completion: {
    ...
    postfixUfcs: setting("completion.postfixUfcs"),
    postfixUfcsLeft: setting("completion.postfixUfcsLeft"),
    postfixUfcsRight: setting("completion.postfixUfcsRight"),
    ...
    const adapter = {
      ...
      async resolveServer(context) {
        ...
        if (!launch) {
          ...
          service.reportMissingServer("ide-tinymist", {
            description:
              "Install [tinymist](https://github.com/Myriad-Dreamin/tinymist) and make sure it is on your PATH, or set its location in the ide-tinymist settings. The editor can also fetch it for you.",
          });
          return null;
        }
