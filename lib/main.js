...
const setting = (key) => lumine.config.get(`ide-tinymist.${key}`);
...
module.exports = {
  consumeIdeClient(service) {
      ...
      async resolveServer(context) {
        ...
        if (!launch) {
          if (!missingReported) {
            ...
            lumine.notifications.addError("Unable to find tinymist", {
              description:
              ...
              buttons: [
                {
                  text: "Install Tinymist",
                  // Progress and failure are reported by the install itself.
                  onDidClick: () => service.installServer("ide-tinymist").catch(() => {}),
                },
              ],
            });
          }
    ...
    const restart = () => {
      for (const session of service.getSessions()) {
        ...
        service.restart(session).catch((error) => {
          lumine.notifications.addError("Unable to restart Tinymist Language Server", {
    ...
    for (const key of ["serverPath", "fontPaths", "systemFonts"]) {
      subscriptions.add(lumine.config.onDidChange(`ide-tinymist.${key}`, restart));
    }
